...
from src.msft.env import END_OF_TEXT_TOKEN
from src.msft.gpt2_training.train_utils import (InputFeatures, InputFeatures_train,
                                       RedditExample)
...
class GPT2FeatureDataset(Dataset):
    ...
    def collate(features):
        ...
        labels = pad_sequence([torch.tensor(f.lm_labels, dtype=torch.long)
                               for f in features],
                              batch_first=True, padding_value=-100) # Tunde
...
def convert_examples_to_features_dynamic(examples, tokenizer,
    ...
    def featurize(example):
        ...
        lm_labels = [-100] * len(context_id) + response_id + [end_of_text_id] + [-100]
