...
def extract_linc_cells(path_in, path_hash, external_keys_pth=None):
	if "valid" in path_hash:
		cells = dict()
		external_keys_pth = external_keys_pth or r"compression/distillation/msft/dstc/data/processed/valid.keys.txt" # TODO: unde
		external_keys = [k.strip() for k in open(external_keys_pth)]	# TODO: Tunde
		for no, line in enumerate(open(path_in, encoding='utf-8')):
			c = line.strip('\n')
			k = external_keys[no]
			cells[k] = [c]
	else:
		external_keys_pth = external_keys_pth or r"compression/distillation/msft/dstc/data/processed/test_real.keys.txt" # TODO: Tunde
		keys = set([line.strip('\n') for line in open(path_hash)])
		cells = dict()
		external_keys = [k.strip() for k in open(external_keys_pth)] # TODO: Tunde
		for no, line in enumerate(open(path_in, encoding='utf-8')):
...
def eval_one_system(submitted, keys, multi_ref, n_refs=6, n_lines=None, clean=False, vshuman=-1, PRINT=True):
	...
	# Tunde
	with open(f"{fld_out}.result", "w") as f:
		f.write(f'n_lines: {n_lines}\n')
		f.write(f'NIST-2: {nist[1]}\n')
		f.write(f'NIST-4: {nist[3]}\n')
		f.write(f'BLEU-2: {bleu[1]}\n')
		f.write(f'BLEU-4: {bleu[3]}\n')
		f.write(f'METEOR: {meteor}\n')
		f.write(f'ENT-4: {entropy[3]}\n')
		f.write(f'DIST-1: {div[0]}\n')
		f.write(f'DIST-2: {div[1]}\n')
		f.write(f'Avg Len: {avg_len}\n')
	# Tunde
	if PRINT:
